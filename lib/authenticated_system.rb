module AuthenticatedSystem
  protected
    ...
    def current_account
      ...
      @current_user = @current_account.identity
    end


    # Store the given user id in the session.
    def current_account=(new_user)
      session[:user_id] = new_user ? new_user.id : nil
      @current_account = new_user || false
    end
    ...
    # Called from #current_account.  First attempt to login by the user id stored in the session.
    def login_from_session
      self.current_account = User.find_by_id(session[:user_id]) if session[:user_id]
    end

    # Called from #current_account.  Now, attempt to login by basic authentication information.
    def login_from_basic_auth
      authenticate_with_http_basic do |login, password|
        self.current_account = User.authenticate(login, password)
      end
    end
    ...
    def logout_keeping_session!
      ...
      @current_account.forget_me if @current_account.is_a? User
