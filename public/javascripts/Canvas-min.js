...
(function(){var Canvas=this.Canvas={version:1.3,author:"Michael Camden",elID:"",el:null,elCtx:null,x:0,y:0,getEl:function(){if(this.el!=null&&typeof(this.el)!="undefined"){return this.el;}
...
var mouseX=parseInt(event.clientX)-parseInt(canvas.x);var mouseY=parseInt(event.clientY)-parseInt(canvas.y);event.mouseX=mouseX;event.mouseY=mouseY;var mouseEventsToFire=new Array();var mouseEvents=new Array("click","dblclick","mousemove","mouseover","mousedown","mouseup");for(i in canvas.ObjectManager.objects){var obj=canvas.ObjectManager.objects[i].object;if(Canvas.util.isOverObj(mouseX,mouseY,obj)&&type=="mousemove"&&typeof(obj._mouseover)=="undefined"){obj._mouseover=true;if(obj.hasEvent("mouseover")){obj.fireEvent(event,"mouseover",canvas.getCtx(),obj);}}
...
for(i in this.objects){if(this.objects[i].objectType==type&&this.objects[i].objectID==objectID){var returnObject={objectAt:i,objectID:i.objectID,objectType:i.objectType,object:i.object};return returnObject;}}
...
return false;},findItem:function(itemID,layerID){if(typeof(itemID)=="undefined"){Canvas.util.debug("LayerManager::findItem; itemID is not defined");return false;}
...
if(typeof(layerID)!="undefined"){var layer=this.findLayer(layerID);if(layer===false){Canvas.util.debug("LayerManager::findItem; layer not found");return false;}else{var item=layer.getItem(itemID);if(item===false){Canvas.util.debug("LayerManager::findItem; item not found");return false;}else{return item;}}}else{for(i in this.layers){var layer=this.getLayerAt(i);var item=layer.getItem(itemID);if(item!==false){return item;}}
...
for(i in this.layers){var layer=this.getLayerAt(i);if(layer.getID()==layerID){return i;}}
...
else{if(!isLayer){this.layers.push(new Canvas.Layer(layer,this));}else{this.layers.push(layer);}}},promoteLayer:function(layerID){if(typeof(layerID)=="undefined"){Canvas.util.debug("Layer::promoteLayer; layerID is undefined");return false;}
...
return false;if(typeof(this.threads)=="object"&&(this.threads instanceof Array)){for(i in this.threads){if(this.getThreadAt(i)!==false&&this.getThreadAt(i).getID()==threadID){return{position:i,obj:this.threads[i]};}}}
...
return this.threads.length;},test:function(){window.console.log('test');}};Canvas.Thread=Canvas.prototype=function(thread){defaultThread={id:"thread_",expires:-1,interval:500,timer:null,init:function(thread){if(typeof(thread)=="undefined"||typeof(thread)!="object"){Canvas.util.debug("Thread::init; thread is not defined, or not an object");return false;}
